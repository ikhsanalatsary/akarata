	...
	SPECIAL_FIRST_ORDER_PREFIX_CHARACTERS.forEach((char) => {
		...
		if (StemmerUtility.isStartsWith(word, wordLength, char) && wordLength > charLength && isVowel(word[charLength])) {
			...
			switch (typeof word != 'undefined') {
				case (_.includes(['meny', 'peny'], char) && !_.includes(AMBIGUOUS_WORDS, word.substring(2))):
					substituteChar = 's';
				break;
				case _.includes(['men', 'pen'], char):
					substituteChar = 't';
					if (_.includes(AMBIGUOUS_WORDS, word.substring(2)) || _.includes(['kah', 'lah', 'ku', 'mu', 'nya', 'kan', 'an', 'i'], word.substring(wordLength -1))) substituteChar = 'n';
				break;
				case char === 'mem' && !_.includes(AMBIGUOUS_WORDS, word.substring(2)):
					substituteChar = 'p';
				break;
				case char === 'pem':
					substituteChar = 'p';
