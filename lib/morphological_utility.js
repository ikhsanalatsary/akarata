...
function substituteChar(AMBIGUOUS_WORD, char, word) {
	...
	switch (typeof word !== 'undefined') {
		case (['meny', 'peny'].includes(char) && !AMBIGUOUS_WORDS.includes(word.substring(2))):
			substituteChar = 's';
		break;
		case ['men', 'pen'].includes(char):
			substituteChar = 't';
			if (AMBIGUOUS_WORDS.includes(word.substring(2))
					|| ['kah', 'lah', 'ku', 'mu', 'nya', 'kan', 'an', 'i'].includes(word.substring(word.length -1))) {
				substituteChar = 'n';
			}
		break;
		case char === 'mem' && !AMBIGUOUS_WORDS.includes(word.substring(2)):
			substituteChar = 'p';
		break;
		case char === 'pem':
			substituteChar = 'p';
